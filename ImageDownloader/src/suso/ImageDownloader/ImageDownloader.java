...
public class ImageDownloader {
	...
	public static void main(String[] args) {
		...
		final String ficheroDeEntrada = "D:\\Borrame.txt";
	...
	private static boolean descargar(String origen, String destino){
		...
		try{
			...
			if (archivo_destino.exists())System.out.println("Ya existe el archivo "+destino);
			else{
				File archivo_temporal=new File(destino+".tmp");
			//Cargamos un objeto URL con la direccion de la imagen que quieras descargar.
			URL url = new URL(origen);
			System.out.println("url: "+url);
			//Abrimos una conexion a esa URL
			URLConnection urlConnection = url.openConnection();
			//Abrimos un stream de entrada para descargarnos la imagen como array de bytes
		    ...
		    dOut.close();
	        bufferentrada.close();
	        System.out.println("TotalSpace: " + archivo_temporal.length());
	        if(archivo_temporal.length()>500) 
	...
	private static void vecinitas_candidatas(String directorio_destino) {
		...
		for (int ano=2010;ano<=2012;ano++){
			for (int candidata=1;candidata<=300;candidata++){ //a partir de 100, dificil
				contador = 0;
				for (int picint=1;picint<=20;picint++){
					String picstr = df.format(picint);
					String archivo = ano + "_candidata_" + candidata + "_" + picstr + ".jpg";
					String url_base="http://especiales.fhm.es/vecinitas" + ano + "/" + candidata + "/normal/normal_"+ picstr +".jpg";
					System.out.println("archivo "+archivo);
					System.out.println("url_base "+url_base);
					if (!descargar (url_base,directorio_destino+archivo)) contador++;
					else contador=0;
					if (contador>3) break;
				}
			}
		...
		for (int ano = 2011; ano <= 2013; ano++) {
			for (int candidata = 1; candidata <= 100; candidata++) {
				...
				for (int picint = 1; picint <= 20; picint++) {
					...
					String url_base = "http://especiales.fhm.es/vecinitas" + ano +"/10finalistas/fotos/" + candidata + "_" + picstr + ".jpg";
					System.out.println("archivo " + archivo);
					System.out.println("url_base " + url_base);
					if (!descargar(url_base, directorio_destino + archivo)) contador++;
					else contador = 0;
	...
	private static void sologatitas(String direccion_hoy, String directorio_destino){
		...
		descargar (url_base+archivo,directorio_destino+archivo);
		}		
	}
	...
	private static void vipchick(String directorio_destino){
		...
		System.out.println("url_base "+url_base);
		String [] meses ={"01-jan","02-feb","03-mar","04-apr","05-may","06-jun",
				"07-jul","08-aug","09-sep","10-oct","11-nov","12-dec"};
		for (int ano=2008;ano<=2009;ano++){
			for (int mes=0;mes<=11;mes++){//es el indice de un array, asi que empieza por 0
				...
				for (int dia=1;dia<=31;dia++){
					...
					System.out.println("archivo "+archivo);
					descargar (url_base+archivo,directorio_destino+archivo_plano);					
				}
			}
		}
	}
	...
	private static void submityourex(String archivo_base, String directorio_destino){
		if(!archivo_base.endsWith("/")) archivo_base += "/";
		...
		for (int picnumber=1;picnumber<=20;picnumber++){
			...
			if (!descargar (url_base + archivo , directorio_destino + archivo_base + archivo)) contador++;
			else contador=0;
			if (contador>3) break;
		}
	}
	...
	private static void fuskator1(String direccion_hoy, String directorio_destino){
		...
		//a partir de la direccion
		//<div class="pic" id="divpic_9d620f9de4c0719b"><div class="pic_pad"><a title="amateur homemade latina shaved solo xlogs"
		//href="/full/hZGb31IUfez/amateur_homemade_latina_shaved_solo_xlogs.html"><img src="/small/hZGb31IUfez/amateur_homemade_latina_shaved_solo_xlogs_4.jpg"
	...
	private static void postyourgirls(String directorio_destino , int offset) {
		...
		// http://www.postyourexgirls.com/enero15/15/02/01.jpg
		String url_base = "http://www.postyourexgirls.com/";
		System.out.println("url_base: " + url_base);

		//String[] meses = {"ene", "feb", "mar", "abr", "may", "jun", "jul", "ago", "sep", "oct", "nov", "dic" };
		String[] meses = {"enero", "febrero", "marzo", "abril", "Mayo", "Junio", "jul", "ago", "sep", "oct", "Noviembre", "Diciembre" };
		...
		for (int dias_antes = 0; dias_antes < 30; dias_antes++) {
			boolean finDeSemana = (calendario.get(GregorianCalendar.DAY_OF_WEEK) == calendario.SATURDAY)|| (calendario.get(GregorianCalendar.DAY_OF_WEEK) == calendario.SUNDAY);
			if (!finDeSemana) {
				...
				for (int pagina = 1; pagina < 11; pagina++){
					String directorio = meses[mesInt] + anoString +
					"/" + diaString + "/" + intToFormatString(pagina, 2) + "/";
					//File f_directorio = new File(directorio_destino + directorio);
					//f_directorio.mkdirs();
					...
					for (int foto = 1; foto < 15; foto++){
						...
						if (!descargar (url_base+archivo,directorio_destino+archivo_plano)) contador++;
						else contador=0;
						if (contador>2) break;
						//descargar (url_base+archivo,directorio_destino+archivo_plano);
						//System.out.println("Origen: " + url_base + archivo);
						//System.out.println("Destino:" + directorio_destino + archivo);
					}
				}
			}
